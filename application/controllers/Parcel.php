...
class Parcel extends CI_Controller {
	...
	{	
		$data= $this->db->select('*')->from('parcel')->order_by('parcel_id','asc')->get()->result(); //here i'm fetching the data form the table
	...
	{	
		...
		$capsule = array('parcelTag'=>$taggingNum,'trackingNum'=>$trackingNo,'custName'=>$cust,'phoneNum'=>$phone, 'courier'=>$cour,'size'=>$size, 'price'=>$price);
